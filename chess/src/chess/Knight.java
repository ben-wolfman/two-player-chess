...
public class Knight extends Piece {
	...
	public Knight(String n, Spot p, String c) {
		super(n, p, c);
	}
	...
	public boolean canMove(Board board, Spot start, Spot end) {
		...
		if (((Math.abs(end.getY()-start.getY()) == 2 && Math.abs(end.getX()-start.getX()) == 1)) || ((Math.abs(end.getY()-start.getY()) == 1 && Math.abs(end.getX()-start.getX()) == 2))) {
			dir = true;
		}
		else {
			dir = false;
